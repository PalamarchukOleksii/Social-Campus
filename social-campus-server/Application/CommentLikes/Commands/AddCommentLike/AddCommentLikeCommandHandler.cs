...
namespace Application.CommentLikes.Commands.AddCommentLike
{
        ...
        ICommentRepository commentRepository) : ICommandHandler<AddCommentLikeCommand>
    {
        ...
        {
            bool isUserExist = await userRepository.IsExistByIdAsync(request.UserId);
            if (!isUserExist)
            {
            ...
            bool isAlreadiAddLike = await commentLikeRepository.IsLike(request.UserId, request.CommentId);
            if (isAlreadiAddLike)
            {
                return Result.Failure(new Error(
